...
import {getQueue} from '../api/atmApi'
import {useDispatch,useSelector} from 'react-redux'
import { listQueue } from '../store/actions/atmAction'
...
const Queue = ()=>{
    ...
    useEffect(()=>{
        async function getData(){
            const resDataQueue = await getQueue()
            dispatch(listQueue(resDataQueue))
        }   
        getData()
    },[])
    const queueState = useSelector((state:IRootState)=>{
        return state.atmData.ListQueue
    })
    return(
        <ScrollView style={{padding:10}}>
            <Text style={{fontSize:30,fontWeight:'bold',color:'black',marginBottom:10}}>List Queue</Text>
            {
                queueState?
                queueState.map((queueItem:any,index:number)=>
                (
                    <View key={index} style={{flexDirection:'row',alignItems:'center',borderBottomWidth:1,paddingBottom:15,marginBottom:15}}> 
                        <Image
                            source={{
                            ...
                            style={{ width: '10%', height: 40 ,borderRadius:20,marginRight:10}}
                        />
                        <View >
                            <Text style={{fontSize:17}}>Name:{queueItem.name}</Text>
                            <Text style={{fontSize:17}}>Transaction:{queueItem.transaction}</Text>
                        </View>
                    </View>
                )):
                <Text>Empty</Text>
